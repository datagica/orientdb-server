...
class Server {
  ...
  constructor(opts) {
    ...
    this.rootPassword = typeof this.users.root !== 'undefined' ? this.users.password : ''
  ...
  start() {
    ...
    return this.configure().then(done => {
      ...
      new Promise((resolve, reject) => {
        ...
        // note: maybe we need to use a "PID range hack" to kill OrientDB and
        // potential child processes? not sure it is multiplatform though.
        // see http://azimi.me/2014/12/31/kill-child_process-node-js.html
        this.process = spawn(this.execPath, [], {
          env: {
            ORIENTDB_HOME: this.runtimePath,
            ORIENTDB_ROOT_PASSWORD: this.rootPassword
            // we could set the JVM path here if we need to
            // JAVA_HOME: ...
          }
        })
